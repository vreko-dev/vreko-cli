#!/usr/bin/env node
import { config, EntitlementsServiceImpl, getBaseUrl, getOrganizationWithPurchasesAndMembersCount } from './chunk-T7FDWUFZ.js';
import './chunk-WD5F2AL4.js';
import './chunk-TNNNH4GI.js';
import { logger } from './chunk-GOYL3F4T.js';
import { combinedSchema, db, apiKeys } from './chunk-S4JUGQ6K.js';
...
function CohortInvite({ email, code, cohort }) {
  const activationUrl = `https://vreko.dev/activate/${code}`;
  const docsUrl = "https://docs.vreko.dev/quick-start";
        ...
        children: [
          ...
          /* @__PURE__ */ jsxs(Text, {
            ...
            children: [
              ...
              cohort,
              ". Use this code to activate your account at",
              " ",
              /* @__PURE__ */ jsx(Link, {
                href: "https://vreko.dev",
                style: {
          ...
          /* @__PURE__ */ jsxs(Section, {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxs(Text, {
                ...
                children: [
                  "After activating, install the CLI and run",
                  " ",
                  /* @__PURE__ */ jsx("code", {
                    style: {
                      color: emailTheme.colors.primary,
...
async function writeAuditLog(eventType, metadata) {
  try {
    const { db: db2, vrekoSchema } = await import('./dist-6FUVLDL5.js');
    if (!db2) {
...
var defaultAPIKeySecurityConfig = {
  livePrefix: "sk_live_",
  testPrefix: "sk_test_",
  keyLength: 32,
  enableScopes: true,
...
var defaultArgon2idConfig = {
  memoryLimit: 65536,
  iterations: 3,
  parallelism: 2,
  type: "argon2id"
};
function generateAPIKey(isLive, config2 = {}) {
  const finalConfig = {
    ...defaultAPIKeySecurityConfig,
    ...config2
  };
...
async function verifyAPIKeyHash(plainKey, hash2) {
  try {
    return await verify(hash2, plainKey);
  } catch (error) {
...
__name(verifyAPIKeyHash, "verifyAPIKeyHash");

// ../../packages/auth/dist/plugins/api-key-plugin.js
var createApiKeyBodySchema = z.object({
  ...
  return {
    ...
    endpoints: {
      createApiKey: createAuthEndpoint("/api-key/create", {
        ...
        const { name, organizationId, permissions, rateLimitEnabled, rateLimitMax, rateLimitTimeWindow, expiresAt, metadata } = ctx.body;
      ...
      verifyApiKey: createAuthEndpoint("/api-key/verify", {
        ...
        const candidates = await db.select().from(apiKeys).where(and(eq(apiKeys.start, start), isNull(apiKeys.revokedAt), or(isNull(apiKeys.expiresAt), gte(apiKeys.expiresAt, /* @__PURE__ */ new Date())))).limit(5);
        let matchedKey = null;
        ...
        }).where(eq(apiKeys.id, matchedKey.id)).catch((err) => {
...
__name(apiKeyPlugin, "apiKeyPlugin");
...
var _auth = betterAuth({
    ...
    session: {
      create: {
        after: /* @__PURE__ */ __name(async (session) => {
          ...
          try {
            const { db: db2 } = await import('./dist-6FUVLDL5.js');
      ...
      create: {
        before: /* @__PURE__ */ __name(async (user2) => {
          ...
          if (!inviteCode && user2.email) {
            const { pendingApiKeys: pendingApiKeysTable, inviteCodes: inviteCodesTable, burnInviteCode } = await import('./dist-6FUVLDL5.js');
            const { and: and3, eq: eq3, gt, isNull: isNull2, sql } = await import('drizzle-orm');
            const claim = await db.select().from(pendingApiKeysTable).where(and3(eq3(pendingApiKeysTable.userId, user2.email), eq3(pendingApiKeysTable.purpose, "pioneer_magic_claim"), gt(pendingApiKeysTable.expiresAt, /* @__PURE__ */ new Date()))).limit(1);
            if (claim.length > 0) {
              const codeRow = await db.select().from(inviteCodesTable).where(and3(sql`lower(${inviteCodesTable.invitedEmail}) = lower(${user2.email})`, sql`${inviteCodesTable.currentUses} < ${inviteCodesTable.maxUses}`, isNull2(inviteCodesTable.revokedAt))).limit(1);
              if (codeRow.length > 0) {
                await burnInviteCode(db, codeRow[0].id);
                await db.delete(pendingApiKeysTable).where(eq3(pendingApiKeysTable.userId, user2.email));
                return {
                  data: user2
                };
              }
          ...
          try {
            const { db: db2 } = await import('./dist-6FUVLDL5.js');
            const { inviteCodes } = await import('./dist-6FUVLDL5.js');
            const { and: and3, eq: eq3, sql } = await import('drizzle-orm');
            const code = await db2.select().from(inviteCodes).where(eq3(inviteCodes.code, inviteCode)).limit(1);
            if (code.length === 0) {
            ...
            const burned = await db2.update(inviteCodes).set({
              ...
              updatedAt: /* @__PURE__ */ new Date()
            }).where(and3(eq3(inviteCodes.id, entry.id), sql`${inviteCodes.currentUses} < ${inviteCodes.maxUses}`)).returning();
            if (burned.length === 0) {
  ...
  onAPIError: {
    onError(error, ctx) {
      ...
      import('./dist-5AAX7VSU.js').then(({ captureError }) => {
...
//# sourceMappingURL=auth-QDSN5C4X.js.map
//# sourceMappingURL=auth-QDSN5C4X.js.map